...
pub fn solve_f2l(cube: &mut RubiksCube, target: &Color) -> Vec<String> {
    ...
    while !colors.is_empty() {
        ...
        for color in colors.clone() {
            ...
            if corner == *target_corner_locations.get(&color).unwrap()
                && edge == *target_edge_locations.get(&color).unwrap()
            {
        ...
        if count == 4 {
            ...
            // slot 1
            if (x_corner == 2 && y_corner == 0 && z_corner == 0)
                || (x_edge == 2 && y_edge == 0 && z_edge == 1)
            {
                // we perform U action if either corner or edge piece is in replacement spot
                if (x_corner == 0 && y_corner == 2 && z_corner == 2)
                    || (x_edge == 1 && y_edge == 2 && z_edge == 2)
                {
                    output_list.push("U".to_string());
                    cube.apply_scramble("U");
                }
            ...
            // slot 2
            if (x_corner == 0 && y_corner == 0 && z_corner == 0)
                || (x_edge == 0 && y_edge == 0 && z_edge == 1)
            {
                // we perform U action if either corner or edge piece is in replacement spot
                if (x_corner == 2 && y_corner == 2 && z_corner == 2)
                    || (x_edge == 1 && y_edge == 2 && z_edge == 2)
                {
                    output_list.push("U".to_string());
                    cube.apply_scramble("U");
            ...
            // slot 3
            if (x_corner == 0 && y_corner == 2 && z_corner == 0)
                || (x_edge == 0 && y_edge == 2 && z_edge == 1)
            {
                // we perform U action if either corner or edge piece is in replacement spot
                if (x_corner == 2 && y_corner == 2 && z_corner == 2)
                    || (x_edge == 1 && y_edge == 2 && z_edge == 2)
            ...
            // slot 4
            if (x_corner == 2 && y_corner == 2 && z_corner == 0)
                || (x_edge == 2 && y_edge == 2 && z_edge == 1)
            {
                // we perform U action if either corner or edge piece is in replacement spot
                if (x_corner == 0 && y_corner == 2 && z_corner == 2)
                    || (x_edge == 1 && y_edge == 2 && z_edge == 2)
...
fn edge_piece_location(cube: &RubiksCube, edge_1: &Color, edge_2: &Color) -> Vec<(usize, usize)> {
    ...
    */
    ...
    for item in &location {
        ...
        let (x, y, z) = local_to_global(item.0, item.1);
        // convert back to local
        let mut local_positions = global_to_local(x, y, z);
        local_positions.retain(|&x| x != *item);
        ...
        if cube.faces[local_positions[0].0][local_positions[0].1] == *edge_2 {
            return vec![*item, local_positions[0]];
        }
    }
...
fn corner_piece_location(
    ...
    */
    ...
    for item in &location {
        ...
        // if the local_position matches the edge_2 color, we return these locations
        if cube.faces[local_positions[0].0][local_positions[0].1] == *corner_2
            && cube.faces[local_positions[1].0][local_positions[1].1] == *target
        {
            return vec![*item, local_positions[0]];
        } else if cube.faces[local_positions[1].0][local_positions[1].1] == *corner_2
            && cube.faces[local_positions[0].0][local_positions[0].1] == *target
        {
            return vec![*item, local_positions[1]];
        }
    }
    vec![(3, 9)]
}
